...
### VI-G. Repository Layer Integrity Principle (NON-NEGOTIABLE)
**数据访问层完整性原则** - Repository层必须100%精准还原：
- **Repository接口完全保持**: 所有Repository接口定义、方法签名、参数类型必须100%保持
- **数据访问模式必须一致**: 同步/异步模式、事务处理、错误处理机制必须完全相同
- **查询语义必须保持**: 查询逻辑、过滤条件、排序规则、分页行为必须保持不变
- **缓存策略必须还原**: 缓存机制、失效策略、缓存键生成逻辑必须完全保持
- **性能特征必须匹配**: 查询性能、响应时间、资源使用必须在可接受范围内
- **并发访问模式**: 并发控制、锁机制、数据一致性保证必须保持一致

**前端项目中的Repository层特殊要求**:
- **API调用模式**: HTTP请求方法、URL路径、请求头、查询参数必须完全保持
- **数据加载策略**: 懒加载、预加载、分页加载等策略必须保持一致
- **错误处理机制**: 网络错误、业务错误、超时处理必须完全相同
- **数据转换逻辑**: 响应数据转换、字段映射、数据格式化必须保持不变
- **状态管理模式**: 数据状态管理、更新机制、响应式行为必须完全保持

**Repository层验证标准**:
- **行为一致性**: 相同输入必须产生完全相同的输出和行为
- **数据完整性**: 数据读写操作必须保持原子性和一致性
- **事务边界**: 事务开始、提交、回滚的时机和范围必须完全保持
- **资源管理**: 连接池、内存使用、文件句柄管理必须保持原有模式

**严禁以下Repository层变更**:
- 修改Repository接口定义或方法签名
- 改变数据访问的同步/异步模式
- 修改查询逻辑或过滤条件
- 改变缓存策略或失效机制
- 调整事务处理逻辑
